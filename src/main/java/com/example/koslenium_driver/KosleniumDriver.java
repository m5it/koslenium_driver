...
public class KosleniumDriver {
	...
	static void runServer(int port, boolean browserMode, String cookieFile, int timeoutSec) throws Exception {
		...
		try (ServerSocket ss = new ServerSocket(port)) {
			...
			System.err.println("koslenium_driver server listening on port " + ss.getLocalPort());
		...
		System.err.println("koslenium_driver server stopped.");
	}
	...
	static Path getJarDir() {
		try {
			Path jarPath = Paths.get(
				KosleniumDriver.class.getProtectionDomain().getCodeSource().getLocation().toURI()
			);
	...
	static void printUsage() {
		System.out.println("Usage: java -jar koslenium_driver.jar [options] <url>");
		System.out.println();
